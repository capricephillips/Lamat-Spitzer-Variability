def plot_sav_files(file):
    ...
    #getting error
    binflux_error = np.std(sav_data['targfcal'] - np.roll(sav_data['targfcal'], 1))/np.sqrt(2)

    binflux_error_array = binflux_error * (0*sav_data['bint'][:,0]+1)
    ...
    plt.title(f"{target_name}", fontsize=19)
